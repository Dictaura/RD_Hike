...
def renew_forbidden_actions(action, seq_, edge_index, forbidden_actions, action_space):
    """
    动作空间 action_space
    :param action: 动作编号
    :param graph: 受动作的图
    ...
    seq_base_list = list(seq_base)

    # 改变本位
    base_old = seq_base_list[place]
    seq_base_list[place] = base_pair[0]
    # 获取邻居
    index = (edge_index[0, :] == place).nonzero()
    pair_places = edge_index[1, index]
    # 寻找氢键并改碱基
    place_h = -1
    pair_base_old = ''
    for pair_place in pair_places:
    ...
    if place_h >= 0:
        ...
        if [base_old, pair_base_old] in base_pair_list:
            ...
            forbidden_change_old_pair = base_pair_list.index([pair_base_old, base_old])
            # 获取旧禁止动作
            forbidden_action_old = (place * action_space + forbidden_change_old)
            forbidden_action_old_pair = (place_h * action_space + forbidden_change_old_pair)
            # 删去旧动作
            # print("f_o: {}, p_f_o: {}".format(forbidden_action_old, forbidden_action_old_pair))
            forbidden_actions.remove(forbidden_action_old)
            forbidden_actions.remove(forbidden_action_old_pair)
        ...
        base_pair_r.reverse()
        forbidden_change_pair = base_pair_list.index(base_pair_r)
        forbidden_action = (place * action_space + forbidden_change)
        forbidden_action_pair = (place_h * action_space + forbidden_change_pair)
        # 加入新动作
        # print("f: {}, p_f: {}\n".format(forbidden_action, forbidden_action_pair))
        forbidden_actions.append(forbidden_action)
        forbidden_actions.append(forbidden_action_pair)
    ...
    # 只考虑本位
        ...
        # 删除旧禁止动作
        for change_old in forbid_changes_old:
            forbidden_action_old = (place * action_space + change_old)
            forbidden_actions.remove(forbidden_action_old)
        # 加入新禁止动作
        for change in forbidden_changes:
            forbidden_action = (place * action_space + change)
            forbidden_actions.append(forbidden_action)
...
def rna_act_pair_hike(action, seq_, edge_index, action_space):
    """
    动作空间 action_space
    :param action: 动作编号
    :param graph: 受动作的图
...
def forbidden_actions_hike(seq_base, dotB, edge_index, action_space):
    if action_space == 4:
        base_pair_dict = base_pair_dict_4
        base_pair_list = base_pair_list_4
    elif action_space == 6:
